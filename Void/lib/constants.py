"""Void-Tools — constants, paths, theme palette."""
import colorsys
import os
...
- Correctif démarrage WinError 1155 (star.PNG sans app par défaut → fallback navigateur)
- Sync remote : manifest bundled prioritaire si config_rev plus récent que le CDN
...
def sp(folder, name):
    return os.path.join(VOID_DIR, "tools", folder, name)


def sp_discord(name):
    return os.path.join(VOID_DIR, "tools", "discord", name)


def sp_roblox(name):
    return os.path.join(VOID_DIR, "tools", "roblox", name)


def sp_social(name):
    return os.path.join(VOID_DIR, "tools", "social", name)


def sp_webhook(name):
    return os.path.join(VOID_DIR, "tools", "webhook", name)
