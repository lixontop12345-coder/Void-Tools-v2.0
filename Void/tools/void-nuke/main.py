...
def _setup_credentials(from_void_tools=False):
    ...
    if bot_token and server_id:
        return
    if from_void_tools:
        print(f"  {R3}[-] token/server missing — configure via Void-Tools (X Config Tkn){RS}")
        sys.exit(1)
    print(f"  {D2}{'─'*46}{RS}")
...
def main():
    ...
    if not CLI_ACTION:
        _boot()
    _setup_credentials(from_void_tools=bool(CLI_ACTION))
    _run_bot()
