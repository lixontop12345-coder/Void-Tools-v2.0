...
ASCII = r"""
  ...
  ╚██╗ ██╔╝██║   ██║██║██║  ██║╚════╝██║██╔═══╝ 
   ╚████╔╝ ╚██████╔╝██║██████╔╝      ██║██║     
    ╚═══╝   ╚═════╝ ╚═╝╚═════╝       ╚═╝╚═╝     
"""
SUBTITLE = "G É O I N T E L L I G E N C E   E T   O R A C L E   R É S E A U   A V A N C É"
...
if __name__ == "__main__":
    try:
        ...
        console.print(Align.center(Panel(
            Text.from_markup("[bold red]VOID-TOOLS[/]  [dim]//[/]  [white]IP ORACLE[/]  [dim]//[/]  [red]DEEP TRACE[/]"),
            border_style="red", padding=(0, 6)
        )))
        console.print()

        console.print(" [bold red]┌─[[/][bold white] IP ou Domaine Cible [/][bold red]][/]")
        ...
        if not target.replace(".","").isdigit():
            try:
                ...
                console.print(f" [dim]Résolu [red]{target}[/] > [white]{resolved}[/]")
                ip = resolved
            except:
                console.print(" [bold red][!] Échec de résolution DNS.")
                ip = target
        else: ip = target

        console.print(f"\n [dim]Lancement de l'Intelligence multi-sources sur [red]{ip}[/]...\n")
        ...
        gtbl.add_row("RÉGION", geo.get("regionName", "—"))
        gtbl.add_row("FAI", geo.get("isp", "—"))
        gtbl.add_row("ASN", geo.get("as", "—"))
        gtbl.add_row("PROXY/VPN", "[bold red]DÉTECTÉ[/]" if geo.get("proxy") else "Non")
        gtbl.add_row("DATACENTER", "Oui" if geo.get("hosting") else "Non")
        
        console.print(Align.center(Panel(gtbl, title="[bold red] GEOLOCALISATION [/]", border_style="red")))

        if ssl_d:
            stbl = Table(box=box.MINIMAL_DOUBLE_HEAD, border_style="red", show_header=False)
            stbl.add_row("NOM COMMUN", ssl_d.get("subject", "—"))
            stbl.add_row("ÉMETTEUR", ssl_d.get("issuer", "—"))
            stbl.add_row("EXPIRE LE", ssl_d.get("expires", "—"))
            stbl.add_row("VERSION TLS", ssl_d.get("ver", "—"))
            console.print(Align.center(Panel(stbl, title="[bold red] CERTIFICAT SSL [/]", border_style="red")))
