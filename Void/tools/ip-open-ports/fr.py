...
def boot():
    if sys.platform.startswith("win"):
        os.system("title VOID // OPEN PORTS SCANNER")
...
def afficher(ip, results):
    ...
    fermes  = [(p, PORTS.get(p, "?"), False) for p, o in sorted(results.items()) if not o]
    ...
    tbl.add_column("STATUT", width=12, justify="center")

    for p, svc, _ in ouverts:
        tbl.add_row(str(p), svc, "[bold bright_green]●  OUVERT[/]")
    for p, svc, _ in fermes:
        tbl.add_row(f"[dim]{p}[/]", f"[dim]{svc}[/]", "[dim red]○  FERMÉ[/]")

    console.print()
    console.print(Align.center(Panel(
        tbl,
        title=f"[bold red]  PORTS SCANNÉS[/]  [dim]─[/]  [white]{ip}[/]  [dim]─[/]  [red]{len(ouverts)} OUVERTS[/] [dim]/ {len(fermes)} FERMÉS[/]",
...
if __name__ == "__main__":
    try:
        ...
        console.print(Align.center(Panel(
            Text.from_markup("[bold red]VOID-TOOLS[/]  [dim]//[/]  [white]OPEN PORTS[/]  [dim]//[/]  [red]TCP VULNERABILITY SCAN[/]"),
        ...
        with Progress(
            ...
            TextColumn("[dim white]Scan TCP en cours...[/]"),
