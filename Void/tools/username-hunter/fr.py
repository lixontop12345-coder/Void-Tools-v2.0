...
def main():
    boot()
    console.print(Align.center(Panel(Text.from_markup(f"[bold red]VOID-TOOLS[/] [dim]||[/] [white]USERNAME HUNTER V2[/]"), border_style="red", padding=(0, 5))))
    console.print()
    
    user = console.input(" [bold red]└─▶[/] [white]Pseudo à pister >> ").strip()
    if not user: return
    ...
    def update(res, count, total):
        ...
        results.append(res)
        current_status = f"Scan de {res[0]}..."
    ...
    with Live(layout, screen=True, refresh_per_second=10):
        while len(results) < len(SITES):
            ...
            table.add_column("PLATEFORME", ratio=1)
            table.add_column("STATUT", ratio=1, justify="center")
            ...
            layout["footer"].update(Panel(Align.center(Text.from_markup(f"[dim]VOID OSINT ENGINE - SCANNING {current_status}[/]")), border_style="red"))
            time.sleep(0.1)

    os.system("cls")
    console.print(Align.center(Panel(Text.from_markup(f"[bold green]RAPPORT TERMINÉ POUR : {user}"), border_style="green", padding=(1, 5))))
    
    final_table = Table(box=box.ROUNDED, border_style="red", expand=True)
    final_table.add_column("Plateforme", style="white")
    final_table.add_column("Résultat", justify="center")
    final_table.add_column("Lien Direct", style="blue underline")
    
    for name, found in sorted(results):
        if found:
            url = [s[1] for s in SITES if s[0] == name][0].format(user)
            final_table.add_row(name, "[bold green]FONCTIONNEL[/]", url)
