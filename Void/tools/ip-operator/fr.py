...
def boot():
    if sys.platform.startswith("win"):
        os.system("title VOID // IP OPERATOR")
...
def afficher(ip, data):
    ...
    console.print(Align.center(Panel(
        tbl,
        title=f"[bold red]  OPÉRATEUR RÉSEAU[/]  [dim]─[/]  [white]{data.get('query', ip)}[/]",
        border_style="red",
        padding=(1, 4),
    )))
    console.print()
...
if __name__ == "__main__":
    try:
        ...
        console.print(Align.center(Panel(
            Text.from_markup("[bold red]VOID-TOOLS[/]  [dim]//[/]  [white]IP OPERATOR[/]  [dim]//[/]  [red]ISP INTELLIGENCE[/]"),
        ...
        with Progress(
            ...
            p.add_task("Extraction des données opérateur...", total=None)
