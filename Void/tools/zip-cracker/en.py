    ...
    pnl = Panel(
        Align.center(Group(
            ...
            Text.from_markup(
                ...
                "[bold #FFD700]VOID PREMIUM[/] license."
            ),
            Text.from_markup("\n[dim]Shop · Discord[/]"),
            Text.from_markup(f"\n[bold #5865F2]{C.SHOP}[/]"),
...
def run_dictionary_attack(filepath, passwords, workers):
    if not is_encrypted(filepath):
        console.print("\n [bold yellow][!][/] This ZIP does not appear to be password protected.")
        ans = console.input(" [bold red]└─▶[/] [white]Continue anyway? (y/n) >> ").strip().lower()
        if ans not in ("y", "yes"):
            return
    ...
    with Progress(
        ...
        task = progress.add_task("Starting...", total=len(passwords))
...
def main():
    ...
    if not zipfile_is_valid(filepath):
        console.print("\n [bold red][x][/] Invalid or corrupted ZIP file.")
        time.sleep(2)
        return
    ...
    console.print(Panel(
        Group(
            Text.from_markup(" [bold red]01  » [/][bold white]Dictionary Attack (Combolist)"),
            Text.from_markup(" [bold red]02  » [/][bold white]Random Bruteforce"),
            Text.from_markup(" [bold #FFD700]07  » [/][bold #FFD700]RAR Cracker [PREMIUM]"),
        ),
        title="[bold red]ATTACK MODULES",
        border_style="red",
        box=box.ROUNDED,
        padding=(1, 2),
    ))
    ...
    if not passwords:
        custom = console.input("\n [bold red]└─▶[/] [white]Combolist folder path >> ").strip().strip('"')
        passwords = load_passwords_from_dir(custom)
    if not passwords:
        console.print("\n [bold red][x][/] No password list loaded.")
        console.print(f" [dim]Place .txt wordlists in:[/] [white]{C.COMBOLIST_DIR}[/]")
        time.sleep(2)
        return
    ...
    if _password_found():
        ...
        console.print(" [bold red][x][/] Failure: password not found in wordlist.")
    time.sleep(2)
