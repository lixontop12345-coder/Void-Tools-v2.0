...
if __name__ == "__main__":
    try:
        ...
        console.print()
        console.print(" [bold red]┌─[[/][bold white] Domaine Cible (ex: monsite.com) [/][bold red]][/]")
        ...
        with Progress(SpinnerColumn(spinner_name="point", style="red"), TextColumn("[dim white]{task.description}"), console=console, transient=True) as p:
            p.add_task(f"Bruteforce en cours sur {target}...", total=None)
        ...
        console.print()
        console.print(f" [bold green][✓] Terminé. {len(found)} sous-domaines trouvés.[/]")
