...
def fetch_json(url):
    try:
        r = requests.get(url, timeout=7, headers={"User-Agent": "void-tools/2.0"})
        return r.json()
    except: return {}
...
if __name__ == "__main__":
    try:
        ...
        console.print(Align.center(Panel(
            Text.from_markup("[bold red]VOID-TOOLS[/]  [dim]//[/]  [white]IP ORACLE[/]  [dim]//[/]  [red]GEOINT[/]"),
            border_style="red", padding=(0, 6)
        )))
        console.print()

        console.print(" [bold red]┌─[[/][bold white] IP ou Nom d'hôte [/][bold red]][/]")
        ...
        console.print(f"\n [dim]Lancement de la trace multi-source sur [red]{ip}[/]...\n")
        ...
        geo = results.get("geo", {})
        console.print(make_table("  Géolocalisation & Réseau", [
            ("IP", ip), ("Pays", geo.get("country")), ("Région", geo.get("regionName")),
            ("Ville", geo.get("city")), ("LAT/LON", f"{geo.get('lat')}/{geo.get('lon')}"),
            ("FAI", geo.get("isp")), ("ASN", geo.get("as")),
            ("Proxy/VPN", "[red]OUI[/]" if geo.get("proxy") else "Non"),
            ("Hébergement", "Oui" if geo.get("hosting") else "Non"),
        ]))

        ssl_i = results.get("ssl", {})
        if ssl_i:
            console.print(make_table("  Sécurité TLS", [
                ("Common Name", ssl_i.get("cn")), ("Émetteur", ssl_i.get("issuer")),
                ("Expiration", ssl_i.get("expires")), ("Version", ssl_i.get("version")),
            ]))
        
        ports = results.get("ports", [])
        if ports:
            ptbl = Table(title="  Services Ouverts", box=box.MINIMAL_DOUBLE_HEAD, border_style="red", show_header=True)
