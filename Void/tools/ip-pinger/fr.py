...
HOST_REGEX = re.compile(r'^[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,}$')
PING_COUNT = 8
...
def boot():
    if sys.platform.startswith("win"):
        os.system("title VOID // IP PINGER")
...
def afficher(host, ip, latencies, tcp_ms):
    ...
    tbl.add_row("CIBLE",         f"[bold bright_white]{host}[/]")
    tbl.add_row("IP RÉSOLUE",    f"[bold white]{ip}[/]" if ip else "[dim red]Résolution échouée[/]")
    tbl.add_row("PAQUETS",       f"{len(latencies)} envoyés")
    tbl.add_row("PERTES",        f"[{loss_color(loss_pct)}]{lost} perdus ({loss_pct}%)[/]")
    tbl.add_row("MIN",           f"[bright_green]{min_ms} ms[/]" if valid else "[dim]—[/]")
    tbl.add_row("MOYEN",         f"[{latency_color(avg_ms)}]{avg_ms} ms[/]" if valid else "[dim]—[/]")
    tbl.add_row("MAX",           f"[{latency_color(max_ms)}]{max_ms} ms[/]" if valid else "[dim]—[/]")
    tbl.add_row("GIGUE",         f"[{latency_color(jitter)}]{jitter} ms[/]" if valid else "[dim]—[/]")
    if tcp_ms:
        tbl.add_row("TCP :80",   f"[cyan]{tcp_ms} ms[/]")
    tbl.add_row("ACTIVITÉ",      bar)

    console.print()
    reachable = "[bold bright_green]  ACCESSIBLE[/]" if valid else "[bold red]  INJOIGNABLE[/]"
    console.print(Align.center(Panel(
        tbl,
        title=f"[bold red]  RAPPORT PING[/]  [dim]─[/]  [white]{host}[/]  [dim]─[/]  {reachable}",
...
if __name__ == "__main__":
    try:
        ...
        console.print(Align.center(Panel(
            Text.from_markup("[bold red]VOID-TOOLS[/]  [dim]//[/]  [white]IP PINGER[/]  [dim]//[/]  [red]LATENCY ANALYSIS[/]"),
            border_style="red", padding=(0, 6)
        )))
        console.print()

        console.print(" [bold red]┌─[[/][bold white] Adresse IP ou Hôte Cible [/][bold red]][/]")
        host = console.input(" [bold red]└─▶[/] [bold white]").strip()

        if not host:
            console.print("\n[red] [!] Aucune cible saisie.[/]")
            sys.exit(0)
        ...
        with Progress(
            ...
            TextColumn("[dim white]Envoi des paquets ICMP...[/]"),
